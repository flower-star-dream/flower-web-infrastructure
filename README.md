...
[![version](https://img.shields.io/badge/version-v1.0.3-blue)](https://github.com/flower-star-dream/flower-web-infrastructure)
...
| 其他无前缀提交                            | 按补丁 +1（建议使用规范前缀） | `1.0.3` → `1.0.4` |

分支规则：

- **开发分支（`dev` / `dev/*` / `dev-*` / `*-dev`）**：release-only 下本地提交不再自动递增版本号，dev 分支版本保持当前状态（如 `1.0.3-dev8` 或与上次发布一致）；如需手动维护 dev 预发布号，可执行 `python -c "import sys; sys.path.insert(0, 'scripts'); from version_bump import *"` 参考计算逻辑，或直接改版本文件（pre-commit 会校验三处一致）。
