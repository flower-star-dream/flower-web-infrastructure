...
[![version](https://img.shields.io/badge/version-v1.0.1-blue)](https://github.com/flower-star-dream/flower-web-infrastructure)
...
| ----------------------------------------- | --------------- | ----------------------------- |
| `feat` / `feat(scope)`                    | 小版本 +1       | `1.0.1` → `1.1.0`             |
| `fix` / `refactor` / `perf` / `test` / `build` / `ci` / `style` | 补丁 +1 | `1.0.1` → `1.0.2` |
| 含 `BREAKING CHANGE:`（footer）或 `!:`（如 `feat!: xxx`） | 大版本 +1 | `1.0.1` → `2.0.0` |
| `docs` / `chore`（纯文档/杂物）           | 不变            | —                             |
| `Merge ...` / revert / squash（无前缀，无法解析） | 跳过，不变 | —                             |
| 其他无前缀提交                            | 按补丁 +1（建议使用规范前缀） | `1.0.1` → `1.0.2` |

分支规则：

- **开发分支（`dev` / `dev/*` / `dev-*` / `*-dev`）**：release-only 下本地提交不再自动递增版本号，dev 分支版本保持当前状态（如 `1.0.1-dev8` 或与上次发布一致）；如需手动维护 dev 预发布号，可执行 `python -c "import sys; sys.path.insert(0, 'scripts'); from version_bump import *"` 参考计算逻辑，或直接改版本文件（pre-commit 会校验三处一致）。
