...
@Description: asyncio 定时任务调度器（规范 §23）：
              ...
              - 执行记录持久化（整改 S23-1）：可选注入 record_store（复用 web_infra.task
...
logger = logging.getLogger("web_infra.schedule")
...
class TaskScheduler:
    ...
    def __init__(
        ...
        :param record_store: 任务执行记录存储（SPI，规范 §23.4 审计留存 ≥90 天，
            复用 web_infra.task 的 TaskRecordStoreInterface；None 表示不记录，向后兼容）
