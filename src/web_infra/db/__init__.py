...
from web_infra.db.mongo_session_interface import MongoSessionInterface
...
from web_infra.db.database_router import DatabaseRouterInterface, TenantDatabaseRouter
from web_infra.db.session_scope_mixin import SessionScopeMixin
from web_infra.db.session_dependency import provide_db_session

if TYPE_CHECKING:
    from web_infra.db.mysql_base import Base
    ...
    from web_infra.db.tenant_query_filter import TenantQueryFilter
    from web_infra.db.database_manager import DatabaseManager

# 惰性导出名 -> 定义子模块（依赖 sqlalchemy/redis/mongo；最小安装未装对应依赖时，
# 仅首次访问该名字才抛 ImportError，`import web_infra` / `from web_infra.db import 非惰性名` 不触发）
_LAZY_EXPORTS: dict[str, str] = {
    "Base": "web_infra.db.mysql_base",
    "MySQLConfig": "web_infra.db.mysql_config",
    "MySQLDatabase": "web_infra.db.mysql_database",
    "SqlAlchemyDatabaseSession": "web_infra.db.sqlalchemy_database_session",
    "MongoDBConfig": "web_infra.db.mongodb_config",
    "BeanieMongoSession": "web_infra.db.beanie_mongo_session",
    "MongoDatabase": "web_infra.db.mongo_database",
    "RedisConfig": "web_infra.db.redis_config",
    "RedisCacheBackend": "web_infra.db.redis_cache_backend",
    "TenantAwareMixin": "web_infra.db.tenant_aware_mixin",
    "TenantQueryFilter": "web_infra.db.tenant_query_filter",
    "DatabaseManager": "web_infra.db.database_manager",
}
...
# TenantQueryFilter / DatabaseManager）不在 __all__ 中——保证 `from web_infra.db import *` 在最小安装
# （未装三者）下不触发惰性导入报错；需使用时请显式导入（如 `from web_infra.db import MySQLConfig`），
# 经模块 __getattr__ 惰性加载。
# 已安装对应依赖（非最小安装）时，下方 _extend_all_with_installed() 会自动将这些名字纳入 __all__，
# 使 `from web_infra.db import *` 全量导出。
