...
@Description: 数据库相关能力统一模块，遵循规范 §3（SPI）、§10（数据访问）、§12.3（分页）、§14.1（连接池）。
              ...
              - 默认实现：MySQLDatabase（SQLAlchemy + aiomysql）
...
# MongoDBConfig / RedisConfig / RedisCacheBackend / TenantAwareMixin / TenantQueryFilter / DatabaseManager）
# 不在 __all__ 中——保证 `from web_infra.db import *` 在最小安装（未装三者）下不触发惰性导入报错；
# 需使用时请显式导入（如 `from web_infra.db import MySQLConfig`），经模块 __getattr__ 惰性加载。
