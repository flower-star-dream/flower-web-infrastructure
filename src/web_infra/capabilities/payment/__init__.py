...
@Description: 支付 SPI 与微信渠道实现聚合导出：统一回调结构、三件 SPI、内存默认实现、
              ...
              微信渠道实现经子模块路径引用（web_infra.capabilities.payment.provider.wechat）；
              对账机制（§6）与风控限额（§9）经 web_infra.capabilities.payment.reconciliation / risk 子包导出。
"""
from web_infra.capabilities.payment.in_memory_payment_callback_verifier import InMemoryPaymentCallbackVerifier
from web_infra.capabilities.payment.in_memory_payment_flow_store import InMemoryPaymentFlowStore
from web_infra.capabilities.payment.in_memory_payment_gateway import InMemoryPaymentGateway
from web_infra.capabilities.payment.in_memory_payment_order_store import InMemoryPaymentOrderStore
from web_infra.capabilities.payment.payment_audit_store import (
...
from web_infra.capabilities.payment.payment_channel_template import PaymentChannelTemplate
...
from web_infra.capabilities.payment.payment_order_store_interface import PaymentLocalOrder, PaymentOrderStoreInterface
from web_infra.capabilities.payment.payment_permission import PaymentPermission
from web_infra.capabilities.payment.payment_reversal import reversal_flow
from web_infra.capabilities.payment.payment_scene import PaymentScene
from web_infra.capabilities.payment.payment_state_machine import PaymentStateMachine
from web_infra.capabilities.payment.payment_status import PaymentEvent, PaymentStatus, RefundStatus
...
from web_infra.capabilities.payment.risk import (
